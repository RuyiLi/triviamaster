...
module.exports = class Trivia extends Command{
  constructor(){
    super('trivia', {
      aliases: ['rivia', 'mc', 'multiple', 'multiplechoice', 'trivia'],
  ...
  async exec(msg, args){
    ...
    if(amount === 'endless' && category !== 'any'){
    ...
    console.log(`Multiple choice trivia game started by ${msg.author.tag}. Request URL: ${url}`);
    ...
    for(const r of res){
      ...
      const _quit = await new Promise((resolve, reject) => {
        const collector = msg.channel.createMessageCollector(m => (m.author.id === msg.author.id && ['1', '2', '3', '4', 'quit', 'skip'].includes(m.content)) || m.member.hasPermission('ADMINISTRATOR'), { time: 30000 });
        ...
        collector.on('collect', m => {
          if(m.content.toLowerCase() === 'quit'){
            ...
            else msg.reply(`Incorrect. You have 1 more attempt.`);
          }
        });
        collector.on('end', (collected, reason) => {
          if(reason === 'correct')
            msg.reply(`Correct! The answer was _**${answers[correct - 1]}**_.`)
          else if(reason === 'incorrect')
            msg.reply(`No more attempts! The correct answer was _**${answers[correct - 1]}**_.`)
          else if(reason === 'time')
            msg.reply(`Time! The correct answer was _**${answers[correct - 1]}**_.`)
          else if(reason === 'quit'){
